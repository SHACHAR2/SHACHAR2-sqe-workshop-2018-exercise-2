...
function WhileStatement (code){
    ...
    let tempToken={'Line':code.loc.start.line, 'Type': 'while statement','Name':'', 'Condition': curTest, 'Value':'', 'Color':''};
    tokens.push(tempToken);
    BlockStatement(code.body);
}
...
function RealValue(code)
{
    ...
    for(i=0;i<names.length;i++)
    {
        if(names[i] in globalVaribles)
            return globalVaribles[names[i]].Value;
    }
    return eval(code);
}
...
{
    let curTest=tempToken.Condition;
...
function ForStatement(code) {
    let curTest = expressions[code.test.type](code.test,false);
    let tempToken = {'Line': code.loc.start.line, 'Type':'for statement','Name':'','Condition':curTest,'Value':'', 'Color':''};
    tokens.push(tempToken);
